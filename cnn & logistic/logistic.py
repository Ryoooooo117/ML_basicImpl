...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(max_iterations):
        ...
        pred = sigmoid(scores)
        loss = pred - y
        gradient_w = np.dot(X.T, loss)/N
        gradient_b = loss.mean()
...
def binary_predict(X, w, b):
    ...
    # print('pred shape X ' + str(X.shape) + ' shape w' + str(w.shape) + ' b ' + str(b))
    preds = np.dot(X,w) + b
    for i in range(N):
        # if preds[i] >= 0:
        if sigmoid(preds[i]) >= 0.5:
            preds[i] = 1
        else:
            preds[i] = 0
...
def multinomial_train(X, y, C, 
    ...
    ny = np.asarray([(y == c).astype(int) for c in range(C)]).T

    for i in range(max_iterations):
        scores = np.dot(X, w.T) + b
        e = np.exp(scores)
        pred = e / np.array([np.sum(e, axis=1)]).T
        loss = ny - pred
        gradient_w = np.dot(loss.T,X) / N
        gradient_b = np.mean(loss, axis=0)
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    # ny = np.array(y)
    ny = np.asarray([(y == c).astype(int) for c in range(C)])
    for c in range(0, C):
        # for i in range(N):
        #     if y[i] == c:
        #         ny[i] = 1
...
def OVR_predict(X, w, b):
    ...
    # nPreds = np.zeros([C, N])
    # for c in range(0, C):
    #     nPreds[c] = np.dot(X, w[c]) + b[c]
    nPreds = np.asarray([np.dot(X, w[c]) + b[c] for c in range(C)])
    preds = np.argmax(nPreds, axis=0)

    # for c in range(0, C):
    #     # print(' x shape:' + str(X.shape) + ' w shape: '+str(w[i].shape) + ' b shape ' + str(b))
    #     nPreds = binary_predict(X, w[c], b[c])
    #     for i in range(N):
    #         if nPreds[i] == 1:
    #             preds[i] = c
        # print('C = ' + str(c))
        # print(preds)
        #print(binary_predict(X, w[i], b[i]))
        #  however, here since binary logistic regression naturally predicts a probability
        #  for each class (recall the sigmoid model), we will simply predict the class with the highest probability
        #  (using numpy argmax).

    # preds_all = np.asarray([np.dot(X, w[k]) + b[k] for k in range(C)])
    # preds = np.argmax(preds_all, axis=0)
