...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        # gradient['W'] = derivative of u(grad) * derivative of w
        self.gradient['W'] = np.dot(X.T,grad)
        # gradient['W'] = derivative of u(grad) * derivative of b
        # gradient['b'] = np.dot(np.ones((1,grad.shape[0])),grad)
        self.gradient['b'] = np.sum(grad, axis=0)
        # gradient['X'] = derivative of u(grad) * W
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        # gradient['X'] = derivative of h(grad) * derivative of x(mask)
